...
// src/app/logistics/LogisticsMap.jsx
'use client';
...
export default function LogisticsMap({ assets, theme }) {
  ...
  useEffect(() => {
    ...
    assets.forEach(asset => {
      const geometry = asset.type === 'truck' ? new THREE.BoxGeometry(2, 1, 1) : new THREE.SphereGeometry(1.5, 32, 32);
      const material = new THREE.MeshBasicMaterial({
        color: asset.status === 'en-route' ? 0x10b981 : asset.status === 'operational' ? 0x3b82f6 : 0xf59e0b
      });
      const mesh = new THREE.Mesh(geometry, material);
      mesh.position.set(asset.coordinates.x, asset.coordinates.z, asset.coordinates.y);
      mesh.userData = { assetId: asset.id, assetName: asset.asset };
      scene.add(mesh);
        ...
        new THREE.SpriteMaterial({
          map: new THREE.TextureLoader().load(
            `data:image/canvas;base64,${(() => {
              ...
              ctx.font = '16px Arial';
              ctx.fillText(asset.asset, 10, 20);
